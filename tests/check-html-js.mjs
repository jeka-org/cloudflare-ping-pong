...
// Get all .ts files in templates directory
const templateFiles = readdirSync(templatesDir).filter(f => f.endsWith('.ts'));

console.log(`Scanning ${templateFiles.length} template file(s) for script blocks\n`);
...
for (const filename of templateFiles) {
  ...
  const knownIssueBlocks = new Set();
  if (filename === 'game.ts') {
    knownIssueBlocks.add(0); // game page uses backtick template literals
  }
  ...
  for (let i = 0; i < scripts.length; i++) {
    ...
    try {
      ...
      if (knownIssueBlocks.has(i) && (err.message.includes('Invalid or unexpected token') || err.message.includes('Unexpected token'))) {
  ...
  if (quoteProblems.length > 0) {
    failures++;
    console.log(`    ✗ Found \\' in onclick handlers (use &quot; instead):`);
...
// Summary
console.log(`${totalScripts} script blocks across ${templateFiles.length} files, ${checks} checks, ${failures} failure(s)`);
if (failures > 0) {
  process.exit(1);
}